...
void StartNode(Config &cfg)
{
    ...
    {
        cfg.logs.node_handle = CreateFileW(
    ...
    wchar_t cmd[] = L"\"node.exe\" --max-old-space-size=192 --max-semi-space-size=16 --expose-gc --permission --allow-fs-read=./* --allow-fs-write=./* --allow-fs-read=C:\\dev_node\\trilium_dev --allow-fs-write=C:\\dev_node\\trilium_dev --allow-child-process --allow-addons .\\apps\\server\\dist\\main.cjs";
    ...
    {
        ok = CreateProcessW(
            NULL, cmd, NULL, NULL, 1,     //(cfg.logs.node == LogMode::FILE_MODE)
    ...
    {
        ok = CreateProcessW(
            NULL, cmd, NULL, NULL, false, //(cfg.logs.node == LogMode::FILE_MODE)

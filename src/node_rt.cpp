...
void StartNode(Config &cfg) {
  ...
  if (cfg.logs.node == LogMode::FILE_MODE) {
    cfg.logs.node_handle =
        CreateFileW(cfg.logs.node_file.c_str(), FILE_APPEND_DATA,
  ...
  wchar_t cmd[] =
      L"\"node.exe\" --max-old-space-size=192 --max-semi-space-size=16 "
  ...
  SetEnvironmentVariableW(L"TRILIUM_DATA_DIR",
                          L"C:\\dev_node\\trilium_dev\\data_sandbox");
  SetEnvironmentVariableW(L"TRILIUM_PORT", L"9090");
  SetEnvironmentVariableW(L"USERPROFILE",
                          L"C:\\dev_node\\trilium_dev\\profile");

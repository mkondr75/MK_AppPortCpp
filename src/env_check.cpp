    ...
    DiskProbeResult &out_result)
{
    ...
    out_result.free_bytes =
        total_number_of_free_bytes.QuadPart;

    out_result.total_bytes =
        total_number_of_bytes.QuadPart;

    return true;
}

bool ProbeRequiredDlls(
...
void PrintProbeResult(const ProbeResult &result)
{
    ...
    }
    {
        std::wstring msg =
            ...
            L"\" diagnostic=\"" + result.webview2.diagnostic_message +
            L"\"";
    ...
    }
    {
        ...
        swprintf_s(
            buffer,
            L"probe.disk free_gb=%.3f total_gb=%.3f",
            free_gb,
            total_gb);

...
static void UpdateControls() {
  BOOL use_system =
      SendMessageW(g_radio_system, BM_GETCHECK, 0, 0) == BST_CHECKED;
...
static LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam,
  ...
  case WM_CREATE: {
    ...
    CreateWindowW(L"STATIC", L"Определите Node.js", WS_VISIBLE | WS_CHILD, 20,
                  20, 400, 20, hwnd, NULL, NULL, NULL);

    std::wstring system_text = g_model->system_node_path + L"  version " +
                               g_model->system_node_version;

    g_radio_system =
        CreateWindowW(L"BUTTON", system_text.c_str(),
                      WS_VISIBLE | WS_CHILD | BS_AUTORADIOBUTTON, 20, 60, 700,
                      20, hwnd, (HMENU)IDC_RADIO_SYSTEM, NULL, NULL);
    // ApplyFont(g_edit_path);
    SendMessageW(g_radio_system, WM_SETFONT, (WPARAM)g_font, TRUE);
    g_radio_custom =
        CreateWindowW(L"BUTTON", L"Использовать пользовательскую Node.js",
                      WS_VISIBLE | WS_CHILD | BS_AUTORADIOBUTTON, 20, 100, 300,
                      20, hwnd, (HMENU)IDC_RADIO_CUSTOM, NULL, NULL);
    // ApplyFont(g_radio_custom);
    SendMessageW(g_radio_custom, WM_SETFONT, (WPARAM)g_font, TRUE);
    g_edit_path =
        CreateWindowW(L"EDIT", g_model->custom_node_path.c_str(),
                      WS_VISIBLE | WS_CHILD | WS_BORDER | ES_AUTOHSCROLL, 40,
                      130, 500, 24, hwnd, (HMENU)IDC_EDIT_PATH, NULL, NULL);
    // ApplyFont(g_edit_path);
    SendMessageW(g_edit_path, WM_SETFONT, (WPARAM)g_font, TRUE);
    CreateWindowW(L"BUTTON", L"...", WS_VISIBLE | WS_CHILD, 550, 130, 40, 24,
                  hwnd, (HMENU)IDC_BUTTON_BROWSE, NULL, NULL);

    CreateWindowW(L"BUTTON", L"OK", WS_VISIBLE | WS_CHILD, 260, 190, 100, 30,
                  hwnd, (HMENU)IDC_BUTTON_OK, NULL, NULL);
  ...
  case WM_COMMAND: {
    ...
    case IDC_BUTTON_OK: {
      ...
      GetWindowTextW(g_edit_path, buffer, 1024);
...
bool ShowNodeQuestion(HINSTANCE hInstance, NodeQuestion &model) {
  ...
  HWND hwnd = CreateWindowExW(
      0, CLASS_NAME, L"Node.js setup", WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU,

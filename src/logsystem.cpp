...
static bool WriteWideLineToHandle(HANDLE h, const std::wstring &text)
{
    ...
    int utf8_size = WideCharToMultiByte(CP_UTF8, 0, text.c_str(), -1, nullptr, 0, nullptr, nullptr);
    if (utf8_size <= 0)
    {
        return false;
    }
    ...
    WideCharToMultiByte(CP_UTF8, 0, text.c_str(), -1, utf8.data(), utf8_size, nullptr, nullptr);
    DWORD written = 0;
...
static void WriteLauncherLog(const wchar_t *level, const wchar_t *msg)
{
    ...
    wchar_t line[4096] = {0};
    swprintf(line, 4096,
             L"{\"ts\":\"%ls\",\"src\":\"launcher\",\"lvl\":\"%ls\",\"msg\":\"%ls\"}\n",
             ts.c_str(),
             level, msg);
    // ========================================================
    // console
...
// public api
// ============================================================
bool InitLogging(LogConfig &cfg)
{ // const
...
{
    WriteLauncherLog(L"info", msg);
}
void LogLauncherError(const wchar_t *msg)
{
    WriteLauncherLog(L"error", msg);
}